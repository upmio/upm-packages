...
listeners=EXTERNAL://0.0.0.0:{{getenv "KAFKA_PORT"}},INTERNAL://0.0.0.0:{{getenv "INTERNAL_PORT"}}
advertised.listeners=EXTERNAL://{{ getv "/nodeport_keepalive_ip" }}:{{ getenv "KAFKA_NODEPORT" }},INTERNAL://{{ getenv "POD_NAME" }}.{{ getenv "SERVICE_NAME" }}-headless-svc.{{ getenv "NAMESPACE" }}:{{getenv "INTERNAL_PORT"}}
...
listener.security.protocol.map=EXTERNAL:PLAINTEXT,INTERNAL:PLAINTEXT
listeners=EXTERNAL://0.0.0.0:{{getenv "KAFKA_PORT"}},INTERNAL://0.0.0.0:{{getenv "INTERNAL_PORT"}}
advertised.listeners=EXTERNAL://{{ getenv "LOADBALANCER_IP" }}:{{ getenv "KAFKA_PORT" }},INTERNAL://{{ getenv "POD_NAME" }}.{{ getenv "SERVICE_NAME" }}-headless-svc.{{ getenv "NAMESPACE" }}:{{getenv "INTERNAL_PORT"}}
inter.broker.listener.name=INTERNAL
{{- end }}

{{- if contains (getv "/external_service_type") "ClusterIP" }}
listener.security.protocol.map=INTERNAL:PLAINTEXT
listeners=INTERNAL://0.0.0.0:{{getenv "INTERNAL_PORT"}}
advertised.listeners=INTERNAL://{{ getenv "POD_NAME" }}.{{ getenv "SERVICE_NAME" }}-headless-svc.{{ getenv "NAMESPACE" }}:{{getenv "INTERNAL_PORT"}}
inter.broker.listener.name=INTERNAL
{{- end }}
...
zookeeper.connect={{ index (jsonArray (getv "/service_zookeeper_array")) 0 }}-svc.{{getenv "NAMESPACE"}}.svc:{{ getv "/zookeeper_zookeeper_port" }}
