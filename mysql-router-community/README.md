...
| `MYSQL-ROUTER_PORT`  | `6446`                  | ❌       | Classic protocol port exposed by Router (template)               |
| `MYSQLX-ROUTER_PORT` | `6447`                  | ❌       | MySQLX protocol port exposed by Router (template)                |

Notes:

- `MYSQL-ROUTER_PORT` 与 `MYSQLX-ROUTER_PORT` 为模板配置键（用于渲染路由端口），非容器入口脚本必需的环境变量名。
- 入口脚本严格校验标记为 Required=✅ 的变量，不满足会退出并给出错误信息。
