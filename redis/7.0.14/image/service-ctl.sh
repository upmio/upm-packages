...
decrypt_pwd() {
  ...
  [[ -f "${secret_file}" ]] || { error "${func_name}" "get file ${secret_file} failed !"; return "${EXIT_GENERAL_FAILURE}"; }

  local enc_iv
  enc_iv=$(head -c 16 "${secret_file}" | od -t x1 -An -v | tr -d ' \n')
  [[ -n "${enc_iv}" ]] || { error "${func_name}" "get enc_iv failed!"; return "${EXIT_GENERAL_FAILURE}"; }

  tail -c +17 "${secret_file}" >"${enc_in}"
  [[ -f "${enc_in}" ]] || { error "${func_name}" "get enc_in failed!"; return "${EXIT_GENERAL_FAILURE}"; }
...
health() {
  ...
  local pong
  if pong=$(redis-cli -h 127.0.0.1 -p "${REDIS_PORT}" ping 2>/dev/null); then
    if [[ "${pong}" == "PONG" ]]; then
      info "${func_name}" "Redis ping OK"
      return 0
    fi
  fi
...
initialize() {
  ...
  # Store local auth for tooling
  local password_config="${CONF_DIR}/.pwd.json"
  {
    echo "{\"redis://127.0.0.1:${REDIS_PORT}\": \"${adm_pwd}\"}"
  } >"${password_config}" || die "${EXIT_CONFIG_FILE_FAILED}" "${func_name}" "create ${password_config} failed!"
...
validate_environment() {
  ...
  [[ -n "${REDIS_PORT:-}" ]] || die "${EXIT_MISSING_ENV_VAR}" "${func_name}" "get env REDIS_PORT failed !"
