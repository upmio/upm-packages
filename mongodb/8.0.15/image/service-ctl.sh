...
readonly EXIT_MONGODB_HEALTH_FAILED=52
...
health() {
  ...
  [[ -n "${adm_pwd}" ]] || die "${EXIT_GENERAL_FAILURE}" "${func_name}" "get ${ADM_USER} password failed!"
	...
	mongosh --username "${ADM_USER}" --password "${adm_pwd}" --eval "db.runCommand({ ping: 1 })" || {
		die 41 "${func_name}" "mongosh db.runCommand({ ping: 1 }) failed !"
	}
}
...
initialize() {
  ...
  if [[ ! -f "${INIT_FLAG_FILE}" ]]; then
    ...
    mongod --fork --dbpath "${DATA_DIR}" --logpath "${LOG_MOUNT}/mongod.log" || {
      die "${EXIT_MONGODB_HEALTH_FAILED}" "${func_name}" "start mongod failed!"
    }

    sleep 2

		local init_js
		init_js="/tmp/init-user.js"
		cat <<EOF >"${init_js}"
...
EOF
    ...
    info "${func_name}" "Creating admin user !"
		mongosh --eval "load('${init_js}')" || {
      die "${EXIT_MONGODB_HEALTH_FAILED}" "${func_name}" "create admin user failed!"
    }
    ...
    if ! pgrep mongod >/dev/null; then
      info "${func_name}" "MongoDB shutdown successfully !"
    else
      die "${EXIT_MONGODB_HEALTH_FAILED}" "${func_name}" "MongoDB shutdown failed !"
    fi

    info "${func_name}" "Creating keyring file !"

    local key_file
		key_file="${DATA_MOUNT}/mongod.key"

    echo -n "${SERVICE_GROUP_NAME}" | base64 >"${key_file}" || {
      die "${EXIT_CONFIG_FILE_FAILED}" "${func_name}" "create ${key_file} failed!"
    }

    chmod 600 "${key_file}" || {
      die "${EXIT_CONFIG_FILE_FAILED}" "${func_name}" "chmod ${key_file} failed!"
    }

    info "${func_name}" "Initialize mongodb done !"
