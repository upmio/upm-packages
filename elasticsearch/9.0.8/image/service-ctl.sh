...
sync_offline_model_bundle() {
  local func_name="sync_offline_model_bundle"

  if [[ "${OFFLINE_E5_ENABLED}" != "true" ]]; then
    info "${func_name}" "Offline E5 bundle sync disabled."
    return
  fi

  [[ -d "${OFFLINE_E5_MODEL_DIR}" ]] || {
    die "${EXIT_DIR_NOT_FOUND}" "${func_name}" "Not found ${OFFLINE_E5_MODEL_DIR} !"
  }

  local bundle_name
  bundle_name="$(basename "${OFFLINE_E5_MODEL_DIR}")"
  local target_dir="${ES_PATH_CONF}/models/${bundle_name}"

  mkdir -p "${target_dir}" || {
    die "${EXIT_DIR_CREATION_FAILED}" "${func_name}" "mkdir ${target_dir} failed!"
  }

  cp -R "${OFFLINE_E5_MODEL_DIR}/." "${target_dir}" || {
    die "${EXIT_GENERAL_FAILURE}" "${func_name}" "copy offline E5 model bundle failed!"
  }

  info "${func_name}" "Prepared offline E5 bundle ${OFFLINE_E5_MODEL_ID} at ${target_dir}"
}
...
initialize() {
  ...
  if [[ ! -f "${INIT_FLAG_FILE}" ]]; then
    ...
    if [[ "${FORCE_CLEAN}" == "true" ]]; then
      rm -rf "${DATA_DIR}" "${ES_PATH_CONF}" || {
        die "${EXIT_DIR_REMOVAL_FAILED}" "${func_name}" "Force remove ${DATA_DIR} ${ES_PATH_CONF} failed!"
      }
    fi
...
validate_environment() {
  ...
  readonly OFFLINE_E5_ENABLED="${OFFLINE_E5_ENABLED:-false}"
  readonly OFFLINE_E5_MODEL_ID="${OFFLINE_E5_MODEL_ID:-.multilingual-e5-small}"
  readonly OFFLINE_E5_MODEL_DIR="${OFFLINE_E5_MODEL_DIR:-/opt/upm/offline-models/multilingual-e5-small}"
}
